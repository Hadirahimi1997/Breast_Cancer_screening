...
patch_left_mlo=np.load('/..../augmented_np_files/aug_left_mlo_patch_5c.npy')
label_left_mlo=np.load('/..../augmented_np_files/aug_left_mlo_label_5c.npy')

################################################################################
################################################################################
patch2 = np.array(patch_left_mlo)
patch2 = np.expand_dims(patch2, axis=3)
X = []
for i in range(len(patch_left_mlo)):
    temp = np.repeat(patch2[i], 3, axis=2)
    X.append(temp)


y_one_hot = np.array(label_left_mlo)
...
for fold, (train_index, val_index) in enumerate(skf.split(X_tr, y_tr)):
    ...
    i+=1
    checkpoint = ModelCheckpoint("/home/workspace/saved_models/Patch_leftmlo2.keras"+format(i), monitor='loss', verbose=1, save_model= True ,save_best_only = True, mode='auto', period=1)
    
    log_dir = "/home/workspace/logs/" + datetime.now().strftime("%Y%m%d-%H%M%S")+" "+"lmlo"+" "+"fold{}".format(i)
