...
patch_right_cc=np.load('/..../augmented_np_files/aug_right_cc_patch_5c.npy')
label_right_cc=np.load('/..../augmented_np_files/aug_right_cc_label_5c.npy')

################################################################################
################################################################################
patch2 = np.array(patch_right_cc)
patch2 = np.expand_dims(patch2, axis=3)
X = []
for i in range(len(patch_right_cc)):
    temp = np.repeat(patch2[i], 3, axis=2)
    X.append(temp)


y_one_hot = np.array(label_right_cc)
...
for fold, (train_index, val_index) in enumerate(skf.split(X_tr, y_tr)):
    ...
    i+=1
    checkpoint = ModelCheckpoint("/home/workspace/saved_models/Patch_rightcc2.keras"+format(i), monitor='loss', verbose=1, save_model= True ,save_best_only = True, mode='auto', period=1)
    
    log_dir = "/home/workspace/logs/" + datetime.now().strftime("%Y%m%d-%H%M%S")+" "+"rcc"+" "+"fold{}".format(i)
